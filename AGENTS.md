...
- **Never compute "today" with `toISOString()`.** Use `oggiRoma()` from `src/lib/date.ts`. `toISOString()` reports UTC, and Rome is UTC+1/+2 — between midnight and 1am (winter) or 2am (summer) local time, `toISOString()` still names the previous day, which would make the freshness script hide today's market as if it had already passed.
